...
def clean_subtitle_misc(text):
    ...
    text = re.sub(r'(?<![？！，哋咁噉你佢我])(?<!^)首先(?![，！？])', r'，首先', text, flags=re.MULTILINE) #I think this will probably get a fair amount of false positives 
...
def update_particle_conventions(text):
    ...
    regex_list_particles = [
        ...
        ('又係嘅', '又係𠸏'),
        ('咩原來', '乜原來'),
        # (r'(?<![係搞])乜([^，？]*?)啊', r'乜\1呀'), # Moving this logic in question recognition
        (r'原來([^，？$]*?)㗎', r'原來\1嘎'),
        ('喇？', '嗱？'),
        (r'即係(.*?)啫', r'只係\1啫'),
        ('呀呢', '啊哩'),
